...
from ctganpc.data_sampler import DataSampler, pc_DataSampler
from ctganpc.data_transformer import DataTransformer
from ctganpc.synthesizers.base import BaseSynthesizer, random_state
        ...
        for i in range(epochs):
            for id_ in range(steps_per_epoch):
                ...
                for n in range(self._discriminator_steps):
                    ...
                    if condvec is None:
                        c1, m1, col, opt = None, None, None, None
                        real, real_parent = self._data_sampler.sample_data(self._batch_size, col, opt)
                    else:
                        c1, m1, col, opt = condvec
                        c1 = torch.from_numpy(c1).to(self._device)
                        m1 = torch.from_numpy(m1).to(self._device)
